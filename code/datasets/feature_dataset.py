...
class FeatureDataset(Dataset):
    def __init__(
        ...
        self.data, self.lengths, self.means, self.stds = self._get_data(
    ...
    def _get_data(self, root: str) -> List[str]:
        ...
        for video_name in tqdm(self.splitnames):
            ...
            if self.indices:
                video_data = torch.arange(0, S, dtype=torch.float32).reshape(
                    S, 1).repeat(1, F) / self.indices_normalizer
