...
class SwinEmbedding(nn.Module):
    ...
    def __init__(self, patch_size=4, C=96):
        ...
        self.linear_embedding = nn.Conv2d(
            3, C, kernel_size=patch_size, stride=patch_size)
...
class ShiftedWindowMSA(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mask:
            x = torch.roll(x, (-self.window_size//2, -
        ...
        if self.mask:
            row_mask = torch.zeros(
                (self.window_size**2, self.window_size**2)).to(get_device())  # .cuda()
            row_mask[-self.window_size * (self.window_size//2):, 0:-
                     self.window_size * (self.window_size//2)] = float('-inf')
            row_mask[0:-self.window_size * (self.window_size//2), -
                     self.window_size * (self.window_size//2):] = float('-inf')
            column_mask = rearrange(row_mask, '(r w1) (c w2) -> (w1 r) (w2 c)',
                                    w1=self.window_size, w2=self.window_size).to(get_device())  # .cuda()
            att_scores[:, :, -1, :] += row_mask
            att_scores[:, :, :, -1] += column_mask

        att = F.softmax(att_scores, dim=-1) @ V
        x = rearrange(att, 'b H h w (m1 m2) E -> b (h m1) (w m2) (H E)',
                      m1=self.window_size, m2=self.window_size)

        if self.mask:
            x = torch.roll(
                x, (self.window_size//2, self.window_size//2), (1, 2))
...
class SwinEncoderBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, window_size, mask):
        ...
        self.WMSA = ShiftedWindowMSA(
            embed_dim=embed_dim, num_heads=num_heads, window_size=window_size, mask=mask)
...
class AlternatingEncoderBlock(nn.Module):
    def __init__(self, embed_dim, num_heads, window_size=7):
        ...
        self.WSA = SwinEncoderBlock(
            embed_dim=embed_dim, num_heads=num_heads, window_size=window_size, mask=False)
