...
# from networks import SpatialPyramidPooling
...
def create_activity_mnist(
    ...
    print(((args.min_segment+args.max_segment)/2)
          * ((args.min_speed+args.max_speed)/2))
    ...
    for target in targets:
        ...
        for (targ, motion, colours) in target:
            # for every sub-task
            subset_list.append((motion, colours, targ, subsets[targ]))
        ...
        for idx in tqdm(range(0, min_len)):  # for every video
            ...
            make_data(videos, labels, database, idx +
                      idx_offset, new_subset_list, video_speed)
    ...
    os.makedirs(f'{args.path}{args.dataset}/rgb-images', exist_ok=True)
    os.makedirs(f'{args.path}{args.dataset}/pooled', exist_ok=True)
    os.makedirs(f'{args.path}{args.dataset}/pooled/small', exist_ok=True)
    os.makedirs(f'{args.path}{args.dataset}/pooled/medium', exist_ok=True)
    os.makedirs(f'{args.path}{args.dataset}/pooled/large', exist_ok=True)
    os.makedirs(f'{args.path}{args.dataset}/labels', exist_ok=True)
    ...
    with open(os.path.join(args.path, args.dataset, 'splitfiles', 'train.txt'), 'w+') as f:
        f.writelines(sorted(video_names[:int(0.9 * len(videos))]))
    with open(os.path.join(args.path, args.dataset, 'splitfiles', 'test.txt'), 'w+') as f:
        f.writelines(sorted(video_names[int(0.9 * len(videos)):]))
    ...
    for video_id, (video, labels) in tqdm(enumerate(zip(videos, labels)), total=len(videos)):
        ...
        os.makedirs(
            f'{args.path}{args.dataset}/rgb-images/{video_id:05d}', exist_ok=True)
        # pooled_small = []
        # pooled_medium = []
        # pooled_large = []
